...
public class ClassListParser implements ICustomParser<List<Class>> {
    /**
     ...
     */
    @Override
    public List<Class> parse(@Nonnull AbstractConfigNode node, @Nonnull String name) throws ConfigurationException {
        try {
            node = node.find(name);
